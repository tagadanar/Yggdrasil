...
import { DemoUserManager, type DemoUser } from './DemoUserManager';
...
export class CleanAuthHelper {
  ...
  async authenticateWithCredentials(email: string, password: string): Promise<CleanAuthResult> {
    try {
      console.log(`🔐 CLEAN AUTH: Starting authentication for: ${email}`);
      ...
      console.log(`🔐 CLEAN AUTH: Auth response status: ${authResponse.status()}`);
      ...
      console.log('🔐 CLEAN AUTH: Authentication and navigation completed successfully');
  ...
  async verifyAuthState(): Promise<boolean> {
    try {
      ...
      console.error('🔐 CLEAN AUTH: State verification failed:', error);
      return false;
    }
  }
