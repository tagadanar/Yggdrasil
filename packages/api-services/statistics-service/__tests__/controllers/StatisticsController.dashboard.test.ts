...
describe('StatisticsController Dashboard API Structure Tests', () => {
  ...
  beforeAll(() => {
    ...
    app.get('/api/statistics/dashboard', StatisticsController.getDashboardStats);
  });
  ...
  describe('Dashboard API Response Structure', () => {
    ...
    it('should return properly structured attendance data array', async () => {
      ...
      // Test each attendance data point
      attendanceData.forEach((point: any) => {
    ...
    it('should return properly structured grade distribution array', async () => {
      ...
      // Test each grade distribution point
      gradeDistribution.forEach((point: any) => {
      ...
      // Verify standard grade letters are present
      const grades = gradeDistribution.map((p: any) => p.grade);
    ...
    it('should return properly structured engagement activities array', async () => {
      ...
      const activityNames = activities.map((a: any) => a.activity);
    ...
    it('should maintain backward compatibility with system overview', async () => {
      ...
      // Should still include systemOverview for backward compatibility
    ...
    it('should return consistent structure across multiple calls', async () => {
      const responses = await Promise.all([
        request(app).get('/api/statistics/dashboard'),
        request(app).get('/api/statistics/dashboard'),
        request(app).get('/api/statistics/dashboard')
      ]);
  ...
  describe('Error Handling', () => {
    it('should handle service errors gracefully', async () => {
      // Mock a service failure by temporarily breaking the random generator
