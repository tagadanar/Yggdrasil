// packages/api-services/statistics-service/__tests__/integration/StatisticsCrossPlatformIntegration.test.ts
...
import { CourseModel, UserModel, NewsModel } from '@101-school/database-schemas';
...
describe('Statistics Cross-Platform Integration Tests', () => {
  ...
  beforeAll(async () => {
    // Setup mock test data across multiple services
    ...
    for (let i = 0; i < 3; i++) {
      const course = {
        ...
        instructor: testUsers[0]._id, // First user is teacher
        duration: { weeks: 12, hoursPerWeek: 4, totalHours: 48 },
        schedule: [],
        capacity: 30,
        enrolledStudents: testUsers.slice(1, i + 2).map(u => u._id), // Enroll different numbers of students
    ...
    // Create mock test news articles
    for (let i = 0; i < 4; i++) {
      const news = {
        _id: `mock-news-${i}-id`,
        ...
        publishedAt: new Date(Date.now() - (i * 7 * 24 * 60 * 60 * 1000)), // Published weekly
        isPublished: true,
        isPinned: i === 0,
        viewCount: (i + 1) * 100 // Different view counts
      };
      testNews.push(news);
    }
  ...
  describe('Cross-Service Statistics Aggregation', () => {
    it('should aggregate statistics from all services', async () => {
      ...
      // Simulate calling a cross-service statistics aggregator that would make these calls
      ...
      expect(aggregatedStats.overview.totalEntities).toBe(
        testCourses.length + testUsers.length + testNews.length
      );
    ...
    it('should generate time-based analytics across services', async () => {
      ...
      // Mock time-based analytics from each service
  ...
  describe('Real-Time Statistics Integration', () => {
    it('should handle real-time statistics updates across services', async () => {
      ...
      // Send real-time events to statistics service
  ...
  describe('Planning and Calendar Statistics Integration', () => {
    it('should aggregate calendar and planning statistics', async () => {
      const mockPlanningData = {
        scheduledClasses: testCourses.length * 2, // Assume 2 classes per course per week
        upcomingEvents: 15,
      ...
      // Mock planning service statistics
    ...
    it('should correlate course enrollments with calendar events', async () => {
      const correlationData = {
        courseEnrollments: testCourses.map(course => ({
          ...
          scheduledHours: 4, // hours per week
          attendanceRate: 0.9
        })),
        calendarEvents: testCourses.map(course => ({
          courseId: course._id,
          eventsScheduled: 24, // events over 12 weeks
          eventsCompleted: 20,
          completionRate: 0.83
        }))
      };
  ...
  describe('Error Handling and Service Resilience', () => {
    it('should handle partial service failures gracefully', async () => {
      ...
      // Verify that statistics can still be generated with partial data
