...
import { ResponseHelper, statsLogger as logger, setupSwagger } from '@yggdrasil/shared-utilities';
import statisticsRoutes from './routes/statisticsRoutes';
import { createStatisticsServiceOpenAPI } from './openapi';

// Load environment variables
dotenv.config();

// JWT system is initialized in index.ts
...
if (process.env['NODE_ENV'] === 'test') {
  ...
  if (!process.env['DB_COLLECTION_PREFIX']) {
    ...
    logger.debug(
      `🔧 STATISTICS SERVICE: Set DB_COLLECTION_PREFIX: ${process.env['DB_COLLECTION_PREFIX']}`,
    );
  }
}
...
if (process.env['NODE_ENV'] !== 'test') {
  const openApiDoc = createStatisticsServiceOpenAPI();
...
app.use((error: any, _req: Request, res: Response, _next: any) => {
  ...
  if (error.name === 'ValidationError') {
    res.status(400).json(ResponseHelper.badRequest('Validation error: ' + error.message));
    return;
  }
  ...
  if (error.name === 'UnauthorizedError' || error.status === 401) {
    res.status(401).json(ResponseHelper.unauthorized('Invalid or expired token'));
    return;
  }

  if (error.status === 403) {
    res.status(403).json(ResponseHelper.forbidden('Access denied'));
    return;
  }
