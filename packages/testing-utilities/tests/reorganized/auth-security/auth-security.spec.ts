...
test.describe('Authentication & Security', () => {
  ...
  test('Password reset flow', async ({ page }) => {
    ...
    try {
      ...
      await page.click('a:has-text("Forgot password?"), a:has-text("Password forgotten?")');
      
      // Step 3: Fill in email for password reset
      await page.fill('input[type="email"]', 'student@yggdrasil.edu');
      await page.click('button[type="submit"]');
      
      // Step 4: Check for success message (in a real app, email would be sent)
      await expect(page.locator('text=reset link has been sent, text=instructions have been sent')).toBeVisible({
        timeout: 10000
      });
