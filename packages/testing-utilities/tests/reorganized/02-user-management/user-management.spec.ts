...
test.describe('User Management - Comprehensive Suite', () => {
  ...
  test('Complete user creation workflow', async ({ page }) => {
    ...
    try {
      ...
      // Fill in the form with unique test data using page.type() for React compatibility
      const timestamp = Date.now();
      const testEmail = `test.user.${timestamp}@yggdrasil.edu`;
      
      // Clear and type into inputs to properly trigger React onChange events
      ...
      // User will be cleaned up automatically by TestCleanup system
  ...
  test('Create form validation and errors', async ({ page }) => {
    ...
    try {
      ...
      await page.locator('[data-testid="create-user-modal"] button:has-text("Create User")').click();
      ...
      // Test 2: Invalid email format - fill all required fields except email is invalid
      ...
      // Wait a bit for React to process and show errors
      await page.waitForTimeout(500);
      
      // Check for the validation error - verify any email error appears
      // The modal should still be visible and have an error for the email field
      await expect(page.locator('[data-testid="create-user-modal"]')).toBeVisible();
      const emailError = page.locator('.form-error').filter({ hasText: /email|Email/ });
      await expect(emailError).toBeVisible({ timeout: 5000 });
      
      // Test 3: Empty password validation
      await emailInput.fill('test@yggdrasil.edu');
      await passwordInput.fill(''); // Clear password field
      ...
      const passwordError = page.locator('.form-error').filter({ hasText: /password|Password/ });
  ...
  test('User edit workflow and validation', async ({ page }) => {
    ...
    try {
      ...
      // Update first name - use page.type() for React compatibility
      const firstNameInput = page.locator('input[name="firstName"]');
      await firstNameInput.clear();
      await firstNameInput.type('EditedTest');
      ...
      // Verify update was successful by checking the updated name appears in the table
      await page.waitForTimeout(1000); // Allow time for table to refresh
      await expect(page.locator('[data-testid="users-table"]:has-text("EditedTest")')).toBeVisible({ timeout: 5000 });
  ...
  test('User deletion with confirmation', async ({ page }) => {
    ...
    try {
      ...
      const factory = new TestDataFactory('USER-DELETE');
      ...
      await deleteButton.click();
      await expect(page.locator('[data-testid="delete-confirmation-modal"]')).toBeVisible();
      await page.locator('[data-testid="delete-confirmation-modal"] button:has-text("Delete")').click();
  ...
  test('Search and filter functionality', async ({ page }) => {
    ...
    try {
      // Create a test user with unique identifier to search for
      const factory = new TestDataFactory('USER-SEARCH');
      const uniqueId = Date.now();
      const testUser = await factory.users.createUser('student', {
        email: `searchtest.${uniqueId}@yggdrasil.edu`,
        profile: {
          firstName: `SearchFirst${uniqueId}`,
          lastName: `SearchLast${uniqueId}`
        }
      });
      ...
      // Test search functionality with our created user's unique email
      const searchInput = page.locator('[data-testid="search-input"]');
      
      // Search for the specific test user we created
      await searchInput.clear();
      await searchInput.type(`searchtest.${uniqueId}`);
      
      // Wait for filtered results (React updates immediately on change)  
      await page.waitForTimeout(1000);
      
      // Verify search finds exactly our test user
      const rowsAfterSearch = await page.locator('[data-testid="user-row"]').count();
      expect(rowsAfterSearch).toBe(1); // Should find exactly 1 user
      
      // Verify it's the correct user
      await expect(page.locator(`[data-testid="user-email"]:has-text("${testUser.email}")`)).toBeVisible();
      
      // Test search with partial name
      await searchInput.clear();
      await searchInput.type(`SearchFirst${uniqueId}`);
      await page.waitForTimeout(1000);
      
      // Should still find our user by first name
      const rowsAfterNameSearch = await page.locator('[data-testid="user-row"]').count();
      expect(rowsAfterNameSearch).toBe(1);
      
      // Test search for non-existent user
      await searchInput.clear();
      await searchInput.type('nonexistentuser99999');
      await page.waitForTimeout(1000);
      
      // Should find no users
      const rowsAfterBadSearch = await page.locator('[data-testid="user-row"]').count();
      expect(rowsAfterBadSearch).toBe(0);
      await expect(page.locator('text=No users match your search criteria')).toBeVisible();
      
      // Clear search - use proper React-compatible clearing
      await searchInput.clear();
      // Trigger change event by typing and deleting to ensure React state updates
  ...
  test('User update API validation', async ({ request }) => {
    ...
    try {
      ...
      const factory = new TestDataFactory('USER-API-UPDATE');
      const testUser = await factory.users.createUser('student');
      cleanup.trackDocument('users', testUser._id);
      ...
      expect(invalidUpdateResponse.status()).toBe(500); // Server error due to duplicate email
