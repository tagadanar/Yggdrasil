...
test.describe('Platform Core - Features, UI States & Profiles', () => {
  ...
  test('Loading and error states across modules', async ({ browser }) => {
    ...
    try {
      ...
      await expect(page.locator('text=Saving, text=Updating'))
        .toBeVisible()
        .catch(() => {});
  ...
  test('Student-specific profile fields', async ({ browser }) => {
    ...
    try {
      ...
      const hasEditFeature = await page
  ...
  test('Navigation consistency and breadcrumbs', async ({ page }) => {
    ...
    try {
      ...
      for (const navItem of navItems) {
        ...
        // Check breadcrumbs
        await expect(page.locator('.breadcrumb, [data-testid="breadcrumb"]'))
          .toBeVisible()
          .catch(() => {});
      }
      ...
      if (await courseLink.isVisible().catch(() => false)) {
        ...
        if (await sectionLink.isVisible().catch(() => false)) {
          ...
          await expect(page.locator('.breadcrumb'))
            .toBeVisible()
            .catch(() => {});
        }
      }
  ...
  test('Sidebar navigation visibility across all pages', async ({ page }) => {
    test.setTimeout(60000);
    const cleanup = TestCleanup.getInstance('PLATFORM: Sidebar Navigation');
    const authHelper = new CleanAuthHelper(page);

    try {
      await authHelper.loginAsStudent();

      // Define all main pages that should have sidebar navigation
      const pagesToTest = [
      ...
      for (const pageTest of pagesToTest) {
        ...
        // Verify Yggdrasil logo is visible in sidebar
        await expect(
          page.locator('[data-testid="sidebar-nav"] img[alt="Yggdrasil Logo"]'),
        ).toBeVisible();
        ...
        // Verify mobile menu toggle is present (for responsive design)
        await expect(page.locator('[data-testid="mobile-menu-toggle"]')).toBeVisible();

        console.log(`✓ Sidebar verified on ${pageTest.name} page (${pageTest.url})`);
      }

      // Test admin-specific pages with admin user
      await authHelper.clearAuthState();
      await authHelper.loginAsAdmin();

      const adminPages = [
        { url: '/admin/users', name: 'Admin Users' },
        { url: '/admin/validation', name: 'Admin Validation' },
      ...
      for (const pageTest of adminPages) {
        ...
        // Verify sidebar is visible on admin pages
        await expect(page.locator('[data-testid="sidebar-nav"]')).toBeVisible({
          timeout: 10000,
        });
        ...
        await expect(page.locator('[data-testid="nav-validation"]')).toBeVisible();

        console.log(`✓ Sidebar verified on admin ${pageTest.name} page (${pageTest.url})`);
      }

      // Test sidebar functionality
      await page.goto('/dashboard');

      // Test sidebar navigation links work
      ...
      if (isMobile) {
        ...
        await expect(page.locator('[data-testid="sidebar-nav"]')).toHaveClass(/translate-x-full/);

        // Click mobile menu toggle
        await page.click('[data-testid="mobile-menu-toggle"]');

        // Sidebar should become visible
        await expect(page.locator('[data-testid="sidebar-nav"]')).toHaveClass(/translate-x-0/);
      }
    } catch (error) {
      await captureEnhancedError(page, error, 'Sidebar Navigation');
      throw error;
    } finally {
      await authHelper.clearAuthState();
      await cleanup.cleanup();
